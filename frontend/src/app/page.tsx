...
const HERO_BG_IMAGE =
  "https://imagedelivery.net/F646Wun-eua00pA0NmkORQ/6d20fedb-67e6-49d2-82b4-c3a37a2d7b00/public";

/** Base URL for HTTP API (create room). When empty, use same-origin /api/rooms. */
const getRoomsUrl = () =>
  GAME_SERVER ? `${GAME_SERVER}/rooms` : "/api/rooms";

export default function HomePage() {
  const router = useRouter();
  const { t } = useTranslations();
  const { muted, toggleMuted } = useSoundStore();
  const { showSettingsHelpModal, setShowSettingsHelpModal, settingsHelpModalTab } = useGameStore();
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const data = (await res.json()) as { roomCode: string; reconnectToken: string };
      sessionStorage.setItem(`reconnect_${data.roomCode}`, data.reconnectToken);
      router.push(`/room/${data.roomCode}?displayName=${encodeURIComponent(name)}`);
    } catch (e) {
      ...
      <div className={styles.container}>
        <header className={styles.header}>
          <div className={styles.logoWrap}>
            <div className={styles.logoIcon}>
              <span className={`material-symbols-outlined ${styles.logoIconSymbol}`} aria-hidden>restaurant</span>
            </div>
          ...
          <div className={styles.nav}>
            <LocaleSwitcher />
            {MUSIC_ENABLED && (
              <button type="button" className={styles.navIconBtn} aria-label={muted ? t("common.unmute") : t("common.mute")} onClick={toggleMuted}>
                <span className="material-symbols-outlined" aria-hidden>{muted ? "volume_off" : "volume_up"}</span>
              </button>
            )}
            <button type="button" className={styles.navIconBtn} aria-label={t("common.settings")} onClick={() => setShowSettingsHelpModal(true, "settings")}>
          ...
          <section className={styles.heroSection}>
            ...
            <div className={styles.heroMascots}>
              <div className={styles.mascot1} aria-hidden>
                <span className={`material-symbols-outlined ${styles.mascotIcon}`}>lunch_dining</span>
              </div>
              <div className={styles.mascot2} aria-hidden>
                <span className={`material-symbols-outlined ${styles.mascotIcon}`}>local_pizza</span>
              </div>
            </div>
            <div className={styles.heroTitleBlock}>
              <div className={styles.skullBox}>
                <span className={`material-symbols-outlined ${styles.skullIcon}`} aria-hidden>skull</span>
              </div>
              <h1 className={styles.heroTitle}>
                {t("common.heroTitleLine1")}
                <br />
                <span className={styles.heroTitleAccent}>{t("common.heroTitleLine2")}</span>
              </h1>
            </div>
              ...
              <div className={styles.formInner}>
                ...
                <div className={styles.actions}>
                  ...
                  <div className={styles.joinDivider}>
                    <span className={styles.joinDividerText}>{t("home.orJoinBattle")}</span>
                  </div>
          ...
          <section className={styles.features}>
            <div className={styles.featureCard}>
              <div className={styles.featureIconWrap}>
                <span className={`material-symbols-outlined ${styles.featureIcon}`} aria-hidden>restaurant_menu</span>
              </div>
            ...
            <div className={styles.featureCard}>
              <div className={styles.featureIconWrap}>
                <span className={`material-symbols-outlined ${styles.featureIcon}`} aria-hidden>swords</span>
              </div>
            ...
            <div className={styles.featureCard}>
              <div className={styles.featureIconWrap}>
                <span className={`material-symbols-outlined ${styles.featureIcon}`} aria-hidden>trophy</span>
              </div>
        ...
        <footer className={styles.footer}>
          <div className={styles.footerLogo}>
            <span className={`material-symbols-outlined ${styles.footerLogoIcon}`} aria-hidden>restaurant</span>
